...
function drawCountries() {
  for (c in countries) {
    ...
    let ctY = map(countryRank,1,countryNumber,y1+20,y2-20)
    fill(255);
    textSize(24);
    text(country,(cnvW*margin)/2,ctY)
  }
}
...
function drawYearDot() {
  ...
  for (y in aY['acq_year']) {
    ...
    if (yr === floor(x)) {
      ...
      let yrY = map(yrCount,maxCount,0,y1+20,y2-20);
      fill(255);
...
function drawYearCountryDot() {
  for (y in aYC['acq_year']) {
    ...
    if (yr === floor(x)) {
      ...
      let ctX = map(yrCountryCount,0,maxYearCountryCount,(cnvW*margin),(cnvW-(cnvW*margin))-20)
